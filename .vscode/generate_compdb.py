...
CMD_VAR_RE = re.compile(
    r'^\s*(?:saved)?cmd_(\S+)\s*:=\s*(.+)\s*$', re.MULTILINE)
...
def gen_compile_commands(cmd_file_search_path, out_dir):
    ...
    if not cmd_files:
        print("No *.o.cmd files found in",
              ", ".join(cmd_file_search_path), file=sys.stderr)
        return
...
def main():
    ...
    cmd_parser.add_argument('cmd_file_search_path',
                            nargs='*', help="*.cmd file search path")
